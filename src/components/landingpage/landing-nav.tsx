...
export default function LandingNav() {
  ...
  return (
    <nav
      className={`fixed left-1/2 z-50 w-full max-w-[92%] -translate-x-1/2 transform px-2 transition-all duration-300 ease-in-out md:max-w-[730px] md:px-3 ${isVisible ? "top-3 md:top-4" : "-top-24"}`}
    >
      <div className="bg-accent/30 border-primary/40 flex items-center justify-between gap-3 rounded-full border px-3 py-2.5 shadow-lg backdrop-blur-md md:gap-4 md:px-4 md:py-3">
      ...
      {isMobile && (
        <div
          id="mobile-menu"
          className={`bg-accent/70 border-primary/30 absolute right-2 mt-2 w-[86vw] max-w-sm rounded-lg border shadow-lg backdrop-blur-md transition-all duration-200 ease-in-out md:right-3 ${isOpen ? "translate-y-0 opacity-100" : "pointer-events-none -translate-y-4 opacity-0"}`}
        >
